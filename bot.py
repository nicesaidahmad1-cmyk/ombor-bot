...
async def db_get(table, params=""):
    async with httpx.AsyncClient() as c:
        r = await c.get(f"{SB_URL}/rest/v1/{table}?{params}", headers=HEADERS)
        return r.json()
...
async def handle_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_user.id == ADMIN_ID:
        if text.startswith("/activate_"):
            ...
            await db_patch("shops", f"telegram_id=eq.{target}", {"active": True})
            ...
            try:
                await ctx.bot.send_message(int(target), "🎉 Hisobingiz faollashtirildi! /start bosing.")
            except:
                pass
            return
    ...
    if state == "reg_nom":
        ...
        if existing:
            ...
            return
        await db_post("shops", {"name": text, "telegram_id": tid, "owner_email": f"{tid}@tg.com", "password_hash": "", "active": False})
    ...
    if text == "📊 Bosh sahifa":
        ...
        await update.message.reply_text(f"ℹ️ *Hisob*\n\n🏪 {shop['name']}\n🆔 `{tid}`\n✅ Faol", parse_mode="Markdown")
    elif isinstance(state, dict):
        step = state.get("step")
        if step == "model_nom":
            user_states[tid] = {**state, "step": "model_olcham", "nom": text}
            await update.message.reply_text("📐 O'lchamini yozing (3-4 yosh, S/M/L yoki o'tkazib yuborish uchun -):")
        elif step == "model_olcham":
            olcham = "" if text == "-" else text
            user_states[tid] = {**state, "step": "model_ranglar", "olcham": olcham}
            await update.message.reply_text("🎨 Ranglar va pochkalarni yozing:\n\n`Qizil - 5\nKo'k - 3\nSariq - 8`\n\nHar qatorga bitta rang!", parse_mode="Markdown")
            ...
            for r in ranglar:
                await db_post("ranglar", {"model_id": model_id, "rang": r["rang"], "stock": r["stock"], "sotildi": 0})
            user_states.pop(tid, None)
            rang_text = "\n".join([f"  • {r['rang']}: {r['stock']}pk" for r in ranglar])
            await update.message.reply_text(f"✅ *{state['nom']}* qo'shildi!\n\n{rang_text}", parse_mode="Markdown", reply_markup=main_menu())
        elif step == "add_stock":
            try:
                soni = int(text)
                r = state["rang"]
                ...
                user_states.pop(tid, None)
                await update.message.reply_text(f"✅ {soni}pk qo'shildi! Jami: {r['stock']+soni}pk", reply_markup=main_menu())
            except:
            ...
            try:
                ...
                await update.message.reply_text(f"✅ Saqlandi!\n*{state['model_nom']}* — {r['rang']}: {soni}pk{warn}", parse_mode="Markdown", reply_markup=main_menu())
            except:
                await update.message.reply_text("⚠️ Son kiriting!")
...
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    data = query.data
    shop = await get_shop(tid)
    if not shop or not shop.get("active"):
    ...
    if data == "new_model":
        ...
        buttons = [
            ...
            [InlineKeyboardButton("📓 Boshqa sana (daftardan)", callback_data=f"sd_{rid}")]
        ]
...
async def admin_list(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        return
    shops = await db_get("shops", "select=*")
    if not shops:
